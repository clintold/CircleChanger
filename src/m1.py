...
class CircleChanger(object):
    ...
    def swallow(self, other_circle_changer):
        ...
        #   NO CREDIT if you use the distance formula here.
        ################################################################
        xcoord = (self.circle.center.x + other_circle_changer.circle.center.x) / 2
        ycoord = (self.circle.center.y + other_circle_changer.circle.center.y) / 2
        rad = self.get_distance_from(other_circle_changer.circle.center) / 2
        colors1 = self.colors + other_circle_changer.colors
        return CircleChanger(xcoord, ycoord, rad, 'red', colors1)
    ...
    def change_to_next_color_in_tuple(self):
        ...
        ################################################################
        self.circle.fill_color = self.colors[self.counter % len(self.colors)]
